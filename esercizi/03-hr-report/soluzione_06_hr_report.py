...
def merge_data(
    ...
    """
    # Merge employees + salaries mantenendo tutti i dipendenti
    df = df_emp.merge(df_sal, on="employee_id", how="left")
    # Merge con performance (sempre left per non perdere dipendenti senza record)
    df = df.merge(df_perf, on="employee_id", how="left")
    # Se il bonus è NaN (nessun record), consideralo 0
    df["bonus"] = df["bonus"].fillna(0)
    # Comp totale = base + bonus
...
def clean_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Converti hire_date a datetime se necessario
    if not np.issubdtype(df["hire_date"].dtype, np.datetime64):
        df["hire_date"] = pd.to_datetime(df["hire_date"], errors="coerce")

    # Rimuovi righe senza base_salary o senza rating (campi critici per KPI/ranking)
...
def aggregate_by_dept_role(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Output:
    - DataFrame aggregato con nomi espliciti
...
def _min_max_norm_by_group(s: pd.Series, g: pd.Series) -> pd.Series:
    """
    Supporto: normalizzazione min-max per gruppo (department).
...
def build_rankings(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    - rating_norm e goals_norm: min-max normalizzati PER department
    ...
    - top_performers_global: prime 10 righe per perf_score DESC
    """
    df = df.copy()

    # Normalizzazioni per reparto
